...
// Main is the application entry point, invoked by cmd/server. It is exported (rather
// than being func main here) because this package is a library: the binary lives in
// cmd/server so the repository root holds a handful of directories instead of eighty
// Go files.
func Main() {
	...
	router.HandleFunc("/api/season-hub/reward-tiers", authMiddleware(requirePermission("view_season_hub", handleSeasonRewardTierList))).Methods("GET")
	router.HandleFunc("/api/season-hub/reward-tiers", authMiddleware(requirePermission("manage_season_rewards", handleSeasonRewardTierCreate))).Methods("POST")
	router.HandleFunc("/api/season-hub/reward-tiers/{id:[0-9]+}", authMiddleware(requirePermission("manage_season_rewards", handleSeasonRewardTierUpdate))).Methods("PUT")
	router.HandleFunc("/api/season-hub/reward-tiers/{id:[0-9]+}", authMiddleware(requirePermission("manage_season_rewards", handleSeasonRewardTierDelete))).Methods("DELETE")
	...
	router.HandleFunc("/api/season-hub/reward-tiers-default", authMiddleware(requirePermission("view_season_hub", handleSeasonRewardTiersDefaultGet))).Methods("GET")
	router.HandleFunc("/api/season-hub/reward-tiers-default", authMiddleware(requirePermission("manage_settings", handleSeasonRewardTiersDefaultPut))).Methods("PUT")
