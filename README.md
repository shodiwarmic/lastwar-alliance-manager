...
A second tab on the **VS** page tracks alliance-vs-alliance Duel League performance — the weekly matchup, not just individual member points — so leadership can understand *why* a week was won or lost, not only that it was.
...
- **Bracket / Match Record**: Capture the full 16-team bracket for a week (each pairing's alliances and scores) and view it as head-to-head bracket pairs plus a derived standings ladder with promotion / demotion zones.
...
- **Opponent scouting**: Optionally paste an opponent's LastRank link to snapshot their power, kills, and member count at matchup time (stored point-in-time so later changes don't rewrite history).
