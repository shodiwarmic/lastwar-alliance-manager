...
- **Where it appears**: The control only shows up when there is something to do — prose already in your language gets none. **This is a desktop-browser feature**: it works in any browser that provides the built-in translator — currently recent Chrome and Edge on desktop — and the control does not appear on phones or tablets. The first translation into a given language downloads that language pack, which can take a while; if it seems stuck, the control will say so and a retry usually works (restarting the browser reliably clears it). The browser also only offers it on a trusted address — an `https://` site, or `localhost`. A normal production install is served over HTTPS and qualifies; reaching a server directly by its LAN IP over plain `http://` does not, so the control will be missing there even on a supported browser. Where it is unavailable, nothing is shown and everything else works exactly as before.
