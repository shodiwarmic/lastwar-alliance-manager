...
- **Member Join Dates**: A member's join date appears as a "Member Since" badge on their roster card and a read-only line on their own profile (shown only when a date is known). Officers with `manage_members` can set or correct a join date from the member edit modal (clearing it sets it back to unknown). New members added via CSV import or LastRank sync can be given a join date per row at confirmation time — CSV imports also auto-read an optional `Join`/`Member Since` column — and left blank, the date defaults to the current game-time date.
...
- **LastRank Prospect Lookup** *(`manage_recruiting`)*: Each prospect/transfer card has a **Look up on LastRank** action that opens a search link for the player, accepts a pasted LastRank profile URL or ID, and fills in power and hero power from [lastrank.fun](https://lastrank.fun) on demand. The pasted ID is saved, so a later **Refresh from LastRank** updates the record in one click — and a per-tab bulk button refreshes every prospect/transfer that has a saved ID at once. Lookups also pull the player's avatar.
