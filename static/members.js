...
function buildMemberCard(member) {
    ...
    rankBadge.className = `member-info-badge rank-${member.rank.replace(/\s+/g, '-')}`;
    ...
    if (member.level) {
        ...
        badge.className = 'member-info-badge badge-hq';
    ...
    if (member.troop_level) {
        const badge = document.createElement('span');
        badge.className = 'member-info-badge badge-troop';
    ...
    if (member.skills) {
        ...
        skillRegistry.forEach(({ key, label }) => {
            if (memberSkills.includes(key)) {
                const badge = document.createElement('span');
                badge.className = 'member-info-badge badge-skill';
    ...
    if (isPowerTrackingEnabled && member.power) {
        ...
        span.className = 'member-info-badge badge-power';
    ...
    // Eligible status — inline with the other stat badges. Train managers can
    // click it to toggle eligibility (no separate action button needed).
    if (canViewTrain) {
        const eligible = member.eligible !== false;
        const span = document.createElement('span');
        span.className = `member-info-badge ${eligible ? 'badge-eligible-yes' : 'badge-eligible-no'}`;
